...
" Last Change:  $Date: 2015/06/97 19:00:00 $
" Filenames:    *.pgsql *.plpgsql
" Note:         The definitions below are for PostgreSQL 9.4, some versions may differ.
" Changelog:    Thanks to
...
syn keyword pgsqlKeyword clob close cluster coalesce cobol collate collation collation_catalog
...
syn keyword pgsqlKeyword general generated get global go goto grant granted greatest group
syn keyword pgsqlKeyword grouping groups

syn keyword pgsqlKeyword handler having header hex hierarchy hold hour

syn keyword pgsqlKeyword id identity if ignore ilike immediate immediately immutable implementation
...
syn keyword pgsqlKeyword into invoker is isnull isolation
...
syn keyword pgsqlKeyword leading leakproof least left level library like like_regex limit
...
syn keyword pgsqlKeyword nfkd nil no none normalize normalized not nothing notify notnull nowait
syn keyword pgsqlKeyword null nullable nullif nulls number object
...
syn keyword pgsqlKeyword table tables tablesample tablespace table_name temp template temporary then
...
syn keyword pgsqlKeyword truncate trusted type types
