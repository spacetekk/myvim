...
exec "Snippet jphpp <?php // joomla<CR><CR>".st.et."<CR><CR>?>"
...
exec "Snippet class class ".st."ClassName".et." ".st."extends AnotherClass".et." {<CR><CR>    function __construct ( ".st."argument".et.") {<CR>".st.et."<CR>    }<CR>}".st.et
...
exec "Snippet func ".st."public".et."function ".st."FunctionName".et." (".st.et.") {<CR>".st.et."<CR>}<CR>".st.et
...
exec "Snippet pd <CR>print_r(".st.et.");die();<CR>"
exec "Snippet checktoken if (!JRequest::checkToken('REQUEST')) {<CR>JError::raiseError(403, JText::_('noauth'));<CR>jexit('invalid token');<CR>}".st.et
exec "Snippet try try {<CR>".st.et."<CR>} catch(Exception $error) { ".st.et."}".st.et
