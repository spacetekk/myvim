...
exec "Snippet from from ".st."module".et." import ".st.'name:PyReplace(@z, "name", "*")'.et
exec "Snippet % '".st."s".et."'".st."s:PyHashArgList(Count(@z, '%[^%]'))".et.st.et
exec "Snippet ass assert ".st."expression".et.st.et
exec "Snippet dr #TODO: remove<CR>raise"
exec "Snippet xx # XXX".st.et
" From Kib2
exec "Snippet bc \"\"\"<CR>".st.et."<CR>\"\"\"<CR>".st.et

" Tekk
exec "Snippet embed #XXX: embed<CR>from IPython import embed<CR>embed()"
exec "Snippet init def __init__(self, *args, **kwargs):
\<CR>return super(".st.et.", self).__init__(*args, **kwargs)"

" Try, except, finally.
exec "Snippet trye try:
\<CR>".st.et."
\<CR>except Exception as error:
\<CR>".st.et."
\<CR>".st.et
...
exec "Snippet imp_ from django.utils.translation import ugettext_lazy as _"
exec "Snippet implog import logging<CR>logger = logging.getLogger('debug')"
